...
class App extends Component {
  ...
  onLayoutChange = (layout, layouts) => {
    // We use isMatch instead of isEqual because `layout` may have certain
    // nested properties with a value of `undefined`, but
    // `this.state.layout` removes these properties completely.
    if (!_.isMatch(layouts, this.state.layouts)) {
      console.log(
        "Setting state: onLayoutChange",
        _.cloneDeep(layouts.md),
        _.cloneDeep(this.state.layouts.md)
      );
  ...
  onGridItemResize = (layout, oldItem, newItem, placeholder) => {
    ...
    if (id === "lexicalItemList") {
      console.log("Forcing LIL update");
      // content.forceUpdate();
    }
  };
   ...
   *
   * Debounced since it will be called every time an item component notifies us
   * of an update, and may therefore be called multiple times within the same
   * frame.
   *
   * @param {...String[]} itemsToCheck - An array containing the IDs of the
   *   items to autosize. Will autosize all items if undefined.
   */
  autosizeGridItems = _.debounce((itemsToCheck) => {
